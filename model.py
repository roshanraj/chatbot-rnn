...
# from tensorflow.contrib.ops import rnn_cell
...
class PartitionedMultiRNNCell(tf.nn.rnn_cell.RNNCell):
...
class Model():
    def __init__(self, args, infer=False): # infer is set to true during sampling.
        ...
        # Call tensorflow library tensorflow-master/tensorflow/python/ops/tf.nn.rnn_cell
        # to create a layer of block_size cells of the specified basic type (RNN/GRU/LSTM).
        # Use the same tf.nn.rnn_cell library to create a stack of these cells
        # of num_layers layers. Pass in a python list of these cells. 
        # cell = tf.nn.rnn_cell.MultiRNNCell([cell_fn(args.block_size) for _ in range(args.num_layers)])
